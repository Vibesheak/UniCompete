...
function VerifyPage() {
  ...
  const handleSendOtp = async () => {
    try {
      const response = await axios.post("http://localhost:8080/auth/resend-verification", { email });
      setMessage({ text: response.data, type: "success" }); // Success message
    } catch (error) {
      setMessage({ text: "Failed to send OTP. Please try again."}); // Error message
    }
  };
  ...
  const handleVerifyOtp = async () => {
    try {
      ...
      Swal.fire({
        icon: "success",
        title: "OTP Verified!",
        text: response.data, // Display the success message from the server
      ...
      Swal.fire({
        ...
        text: error.response?.data || "Invalid OTP. Please try again.",
        confirmButtonColor: "#d33",
      });
  
      setMessage(error.response?.data || "Invalid OTP. Please try again.");
    }
  };
